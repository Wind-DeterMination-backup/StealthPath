...
## 🚀 v5.2.2
- 🧭 修复“跨矿渣 + 深水时路径卡死”问题：目标候选点改为在可配置半径内收集并按距离排序，不再只盯最近一圈，避免深水附近无解导致整条路径失败。
- 🌊 深水判定重规划增强：偏移路径一旦判定会淹没，会从偏移起点重新寻路，而不是直接丢弃该候选。
- ⚙️ 新增高级设置 `目标候选半径`（默认 24 格），可按地图复杂度调大，提升深水/液体边缘场景的规划成功率。
- 🧪 优化沉没与地块伤害估算：统一使用更稳定的单位速度基准，减少对当前脚下地块状态的偏差放大；同时优化沉没阻断收集逻辑，降低大液体区的计算开销。
